...
export class ListCreateUseCase implements IListCreateUseCase {
  ...
  public async execute(listCreateRequestDTO: IListCreateRequest): Promise<IListCreateResponse> {
    ...
    const listExists = await this.listRepo.listGetOne({ listName, userId: session?.id });
    ...
    const createdList = await this.listRepo.listGetOne({ listId: Number(response?.listId) });
